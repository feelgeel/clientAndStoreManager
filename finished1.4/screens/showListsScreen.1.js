...
import *as listNamesApi from "../api/listNameApi";
...
function ShowListsScreen({ navigation,route }) {
  ...
  const getProducts=async()=>{
    if(prod.length==0){
      ...
      const {data:listNames}=await listNamesApi.loadListNames(userId);
      context.addCateg(categ);
      context.setCategStore(categ);
      dispatch(listNamesAction.loadListNames(listNames));
      setProd(categ)
      setloading(false)
