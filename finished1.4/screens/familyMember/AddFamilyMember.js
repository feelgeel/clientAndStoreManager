...
import { handleAddFamilyMember } from './addFamilyFuncs';
...
const AddStoreschema = Yup.object().shape({
    ...
    familyMember: Yup.object().required().label("familyMember"),
  })
function AddFamilyMember({children,style}) {
    const[scanQrModal,setscanQrModal]=useState(false) 
    const[addFamModal,setaddFamModal]=useState(false)
...
<C_Button
        title="add family memeber"
        // color={buttonColor}
        onPress={()=>setaddFamModal(true)} />
<C_Button
...
<C_Form
    ...
    onSubmit={(values)=>{
        // console.log(values)
        handleAddFamilyMember(values,family,user,setaddFamModal)
