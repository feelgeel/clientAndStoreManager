...
import { AddOrderProd, UpdateListOrderProd,
   RemoveListOrderProd } from "../../api/storeListOrderProdApi";
import *as storeMaualorderListAction from "../../redux/storeMaualorderList";
import *as stockListAction from "../../redux/StoreStock";
import { addStock, getStock, getManualorderStock,
   updateStock, deleteStock,getUserStock } from "../../api/stockApi";
...
export const handleAddToChosen=(theChosen,selectedListName,
    ...
    product,setproduct,
    chosen,setchosen,user,price,benefit,stockAlert,values)=>{
 ...
 export const handleUpdateManualListAndProd=async(chosen,user,setmanualOrderLists,
  ...
  setselectedListName,selectedListName,theChosenRedux)=>{
      ...
      if(index!==-1){
          if(stockDb.ok){
            ...
            // if(dt.quantity!==selectedChosen.quantity){
              ...
              await UpdateListOrderProd(newDt._id,selectedChosen)
              let {data:updateStockDb}=await updateStock(stockDb.data[0]._id,
                stockDb.data[0])
              
              }
        }else{
         const {data:delete_manual_order_prod}=await RemoveListOrderProd(dt._id)
    ...
    chosen.map(async(dt)=>{
      ...
      if(index==-1){
        const {data:add_manual_order_prod}=await AddOrderProd(dt)
        let stock={
...
setproduct([])
 ...
 export const handleUpdateTheChosenQuant=(quantity,price,
  settheChosen,chosen,setchosen,theChosen,benefit,stockAlert,values)=>{
 ...
 export const handleAddScannedProd=async(theChosen,scannedProd,quantity,price,
  setchosen,user,chosen,setproduct,benefit,stockAlert,selectedProd,values)=>{
if(selectedProd._id){
    let newProd={...selectedProd}
let newChosen=[...chosen]
newProd.quantity=values.quantity;
newProd.userId=user.userId;
newProd.productId=newProd._id;
let index=chosen.findIndex(dt=>dt._id===selectedProd._id)
if(index==-1){
  newChosen.push(newProd)

}else{
  let daChosen=newChosen[index]
  let newQuantity=Number(newProd.quantity)+Number(daChosen.quantity)
...
export const handleGetStock=async(user,setproduct,chosen)=>{
