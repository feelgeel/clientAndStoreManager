...
/>
<C_Button
title="exit ListOrderScreen mode"
onPress={()=>{
    // context.setModes("")
...
buttonColor="#dc3545"
manualOrder={false}
onAddStockAlert={()=>handleGetStock(user,setproduct,chosen)}
//add quantity
...
onAddQuantity={(values)=>handleAddToChosen(
    ...
    user,price,benefit,stockAlert,values)}
...
onUpdateTheChosenQuant={(values)=>handleUpdateTheChosenQuant(quantity,price,
...
onScan={(values)=>handleScannedGting(scannedGting,setscannedProd,
    setscannedProdModel,user,settheChosen,theChosen,chosen,dispatch,
    setscannedgtingResChosen,setscannedgtingResProd,setselectedProd,values)}
//scanned prod
